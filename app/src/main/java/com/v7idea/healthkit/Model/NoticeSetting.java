...
public class NoticeSetting {
    private final static String TAG = "NoticeSetting";

    public ApiResult getNoticeSetting(String tokenID) {
        String api = APIFetch.getNoticeSettingApi();
    ...
    public ApiResult getEditNoticeSetting(String tokenID, String noticeKey, String noticeSetting) {
        String api = APIFetch.getEditNoticeSettingApi();
        DownLoad httpUtils = new DownLoad();


        List<V7NameValuePair> content = new ArrayList<V7NameValuePair>();
        content.add(new V7BaseNameValuePair("noticeKey", noticeKey));
        content.add(new V7BaseNameValuePair("noticeSetting", noticeSetting));

//        V7HttpResult httpResult = httpUtils.getStringFromURLByPostAddHeader(api, DownLoad.HeaderKeyName, tokenID
//                , DownLoad.formDataContentType, content);

        V7HttpResult httpResult = httpUtils.postUrlByFormData(DownLoad.HeaderKeyName, tokenID
                , api, content);
        DebugLog.d(TAG, "httpResult code: " + httpResult.getResponseCode());
        DebugLog.d(TAG, "httpResult result: " + httpResult.getResultString());
